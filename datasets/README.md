...
| Date | Datetime | Raw Date and Time will not be used directly, derived features such as hour, minutes, day of the week, or month will be extracted | Keep but need Feature Engineering |
| Time | Datetime | Raw Date and Time will not be used directly, derived features such as hour, minutes, day of the week, or month will be extracted | Keep but need Feature Engineering |
| Vehicle_Type | Categorical | Classification analysis; encode into 2W, 3W, LMV, HCVP, HCVG | Keep but need Encoding |
...
| Driver_Gender | Categorical | Classification analysis; encode into Male, Female, Others | Keep but need Encoding |
| License_Type | Categorical | Classification analysis; encode into Learner, Permanent, Heavy, Commercial | Keep but need Encoding |
| Penalty_Points | Numerical | Numerical Feature Capturing on violation severity | Keep as it is |
| Weather_Condition | Categorical | Classification analysis; encode into Clear, Rainy, Foggy, Cloudy, Dust Storm | Keep but need Encoding |
| Road_Condition | Categorical | Classification analysis; encode into Dry, Wet, Potholes, Slippery, Under Construction | Keep but need Encoding |
| Officer_ID | Categorical | Categorical feature indicating officer ID | Can be Dropped |
...
| Traffic_Light_Status | Categorical | Classification analysis; encode into Red, Yellow, Green | Keep but need Encoding |
